...
| `GET` | `/api/v1/class-subjects/students/{studentId}/exemptions` | Dispenses d'une matière OBLIGATOIRE (Évolution « Dispenses ») : `{ studentId, schoolYearId, subjects: [{ subjectId, name, isExempt, reason, gradeCount }] }` — les matières dispensables de la classe de l'élève, l'éventuelle dispense de l'année active avec son motif, et le nombre de notes de l'année qu'une dispense masquerait. **Directeur, Secrétariat seulement** (le motif peut être médical). `403` (autre rôle), `404` (élève inconnu ou d'une autre école), `422` (aucune année active) |
| `PUT` | `/api/v1/class-subjects/students/{studentId}/exemptions` | **Remplace** l'ensemble des dispenses de l'élève pour l'année active : `{ exemptions: [{ subjectId, reason }] }`, liste vide = plus aucune. Une transaction : retrait par suppression logique, motif modifié mis à jour, nouvelles dispenses ajoutées. `204` ; `403` ; `404` ; `409` (collision sur l'index unique, deux saisies simultanées) ; `422` (clé `Exemptions` : motif vide ou de plus de 200 caractères, matière non dispensable — option, domaine, matière d'une autre classe —, doublon ; clé `StudentId` : élève sans classe ; aucune année active). Les messages nomment la matière, jamais le motif (Directeur, Secrétariat) |
...
**Effets d'une dispense sur les autres routes.** Une matière dispensée sort de `GET /grades/summary` (et donc des bulletins, de la délibération et de la fiche élève) ; `GradeSummaryDto.ExemptSubjects` la liste avec son coefficient effectif, **sans jamais le motif**, et `ReportCardDto.ExemptSubjects` la reporte au bulletin. `POST /grades` refuse une note pour un élève dispensé (`422` : « Cet élève est dispensé de cette matière ») et l'import d'une feuille de notes rejette la ligne avec le même message. **`PUT` et `DELETE` d'une note déjà saisie ne sont pas bloqués** : la note reste jointe par l'API mais est invisible dans les moyennes et les bulletins. La grille de saisie, la fiche papier et le modèle Excel de la matière n'incluent plus l'élève dispensé.
