...
- `classrooms.Series` — `varchar(10)`, nullable, sans défaut. Code du catalogue fermé `LyceeSeries` (séries du Baccalauréat — `L1A`, `L1B`, `L'1`, `L2`, `S1` à `S5`, `STEG`, `T1`, `T2`, `STIDD`, `LA`, `S1A`, `S2A` — et les anciens codes `L1`, `TECH`) ; null pour toute classe sans série. Refusée à l'écriture hors d'une classe de Lycée. L'élève hérite la série de sa classe.
...
**Séries, matières par classe et options (Évolution N°6).** Deux tables tenant, sans coefficient propre (le coefficient reste celui du moteur ci-dessus) :

- `class_subjects` — programme d'une classe : `Id`, `SchoolId`, `ClassroomId`, `SubjectId`, `OptionGroup` (`varchar(40)`, nullable : null = matière suivie par toute la classe), `IsCustom` (ajoutée par le Directeur), `IsActive` (défaut `true`), `DisplayOrder`, colonnes d'audit et de suppression logique, `xmin` (`409`). Index unique **partiel** `(ClassroomId, SubjectId) WHERE NOT "IsDeleted"`. FK composites `(SchoolId, …)` vers `classrooms` et `subjects` (`RESTRICT`). Non rattachée à une année. Alimentée depuis le modèle national de la série à la création de la classe (`ClassSubjectTemplateInjector`).
- `student_subject_enrollments` — option retenue par un élève pour une année : `Id`, `SchoolId`, `StudentId`, `ClassSubjectId`, `SchoolYearId`, audit, suppression logique, `xmin`. Index unique **partiel** `(StudentId, ClassSubjectId, SchoolYearId) WHERE NOT "IsDeleted"`. FK composites vers `students`, `class_subjects`, `school_years` (`RESTRICT`). Un changement d'option archive l'ancien choix.
- `student_subject_exemptions` — dispense d'un élève pour une matière OBLIGATOIRE, pour une année (Évolution « Dispenses ») : `Id`, `SchoolId`, `StudentId`, `SubjectId`, `SchoolYearId`, `Reason` (`varchar(200)`, **non nul** : motif obligatoire, donnée sensible), audit et suppression logique. Index unique **partiel** `UX_student_subject_exemptions_key` `(SchoolId, StudentId, SubjectId, SchoolYearId) WHERE NOT "IsDeleted"` (refaire la liste recrée une dispense retirée). FK composites `(SchoolId, StudentId)`, `(SchoolId, SubjectId)`, `(SchoolId, SchoolYearId)` vers `students`, `subjects`, `school_years` (`RESTRICT`). Rattachée à l'élève et à l'année, pas à l'inscription. **Pas de `xmin`** : l'écriture (`PUT`) est un remplacement idempotent de l'ensemble des dispenses de l'élève pour l'année. RLS + Global Query Filter, `GRANT SELECT, INSERT, UPDATE` (aucun `DELETE`). Purges : `student_subject_exemptions` avant `enrollments` dans `reset_school_data` (ses parents `students`, `subjects` et `school_years` viennent après) et avant la suppression des inscriptions dans `delete_school_year`, pour l'année supprimée seulement. Migrations `AddStudentSubjectExemptions` et `AddStudentSubjectExemptionsToPurges`.

Les deux tables : RLS + Global Query Filter, `GRANT SELECT, INSERT, UPDATE` seulement (aucun `DELETE`). Purges : `student_subject_enrollments` avant `students` et `class_subjects` avant `subject_coefficient_overrides` dans `reset_school_data` ; `student_subject_enrollments` dans `delete_school_year` (avant l'année). Règle de suivi (`SubjectFollowRules`) : un élève ne suit pas une matière désactivée de sa classe, ni une matière d'un groupe d'options qu'il n'a pas choisie ; tout le reste — y compris toute matière d'une classe sans programme — est suivi.
