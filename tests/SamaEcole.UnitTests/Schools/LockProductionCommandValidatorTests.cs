...
/// </summary>
public class LockProductionCommandValidatorTests
{
    private readonly LockProductionCommandValidator _validator = new();

    [Theory]
    [InlineData("")]
    [InlineData("   ")]
    public void An_Empty_Confirmation_Should_Be_Rejected(string confirmation)
        => _validator.Validate(new LockProductionCommand(confirmation)).IsValid.Should().BeFalse();

    [Fact]
    public void A_Filled_Confirmation_Should_Pass_The_Validator()
        => _validator.Validate(new LockProductionCommand(LockProductionConfirmation.Keyword)).IsValid.Should().BeTrue();

    [Fact]
    public void A_Wrong_Word_Should_Pass_The_Validator_And_Be_Refused_By_The_Handler()
        => _validator.Validate(new LockProductionCommand("n'importe quoi"))
