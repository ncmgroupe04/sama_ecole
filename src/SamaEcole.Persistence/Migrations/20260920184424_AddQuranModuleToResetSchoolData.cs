...
{
    /// <summary>
    /// Ajoute <c>quran_progress</c> et <c>quran_evaluations</c> (migration AddQuranCoreModule) à
    /// <c>v_targets</c> — deux FK ON DELETE RESTRICT vers <c>students</c> que reset_school_data
    /// ignorait, exactement le bug documenté par
    /// <c>Every_Restrict_Foreign_Key_Into_A_Purged_Table_Must_Come_From_A_Purged_Table_Too</c> (voir
    /// FixResetSchoolDataMissingChildTables et AddClassJournalToResetSchoolData, qui ont corrigé le
    /// même oubli pour Examens, Intégration étatique, Inventaire et Cahier de texte). Placées AVANT
    /// students dans la boucle de purge, comme l'exige la clé étrangère.
    ///
    /// AGENTS.md règle « ne jamais modifier une migration déjà appliquée » : on ne touche pas
    /// DecoupleProductionLockFromLiveMode, on la corrige par cette nouvelle migration. Le corps de la
    /// fonction est repris à l'identique par ailleurs (gardes IsProductionLocked/WentLiveAt).
    /// </summary>
    public partial class AddQuranModuleToResetSchoolData : Migration
    {
        /// <summary>Périmètre complet, identique à DecoupleProductionLockFromLiveMode + quran_progress/quran_evaluations.</summary>
        private const string Targets = """
                    v_targets text[][] := ARRAY[
                        ...
                        -- Socle Coran/Franco-Arabe (AddQuranCoreModule) : FK RESTRICT vers students.
                        ['quran_evaluations',           'Notes d''examen oral coranique'],
                        ['quran_progress',              'Suivi de mémorisation coranique'],
        ...
        /// <summary>Périmètre PRÉCÉDENT (sans quran_progress/quran_evaluations), pour Down().</summary>
        private const string TargetsBeforeQuran = """
                    v_targets text[][] := ARRAY[
                        ['payment_breakdowns',          'Ventilations de paiement'],
                        ['payments',                    'Paiements'],
                        ['debtor_reminder_batch_items', 'Relances de débiteurs (lignes)'],
        ...
        /// <summary>Nettoyage des comptes du personnel, identique à DecoupleProductionLockFromLiveMode.</summary>
        ...
        /// <summary>Corps commun des gardes (IsProductionLocked/WentLiveAt), identique en Up() et Down().</summary>
        ...
        {
            // Restaure le corps exact de DecoupleProductionLockFromLiveMode (sans quran_progress/
            // quran_evaluations) — cette migration-là reste, elle, inchangée.
            migrationBuilder.Sql($$"""
                CREATE OR REPLACE FUNCTION reset_school_data(p_school_id uuid)
                RETURNS TABLE (label text, rows_deleted integer)
                LANGUAGE plpgsql
