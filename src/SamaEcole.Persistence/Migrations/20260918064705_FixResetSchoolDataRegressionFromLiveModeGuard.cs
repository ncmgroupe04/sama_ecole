...
{
    /// <summary>
    /// Corrige une régression introduite par AddHasEverGoneLiveLock (20260918013557).
    ///
    /// Cette dernière a réécrit <c>reset_school_data</c> avec un <c>CREATE OR REPLACE FUNCTION</c> basé
    /// sur l'état ANTÉRIEUR à ExtendResetSchoolDataToConfiguration (20260915230658) — écrite avant que
    /// cette extension n'existe sur la branche, puis rebasée par-dessus sans que le corps SQL, lui, ne
    /// soit remis à jour (aucun conflit textuel : deux fichiers de migration distincts, donc rien à
    /// signaler à la fusion). Le résultat, appliqué dans cet ordre, effaçait silencieusement TOUT le
    /// périmètre ajouté le 15/09/2026 : classes, matières, enseignants et leurs affectations, emploi du
    /// temps, pointages, contrats et fiches de paie, déclarations fiscales, barème des frais, inventaire,
    /// ET la suppression des comptes du personnel (v_staff, détachement du journal d'audit) — la « remise
    /// à neuf » du mode test redevenait celle d'avant le 15/09/2026, sans que le Directeur ni personne
    /// ne le sache avant que les tests d'intégration ne l'attrapent au premier passage de CI sur la
    /// branche fusionnée.
    ///
    /// Cette migration réapplique le corps EXACT de ExtendResetSchoolDataToConfiguration — Targets,
    /// UPDATE ParentSubjectId, boucle de purge, suppression des comptes du personnel — et y substitue
    /// SEULEMENT la garde : <c>HasEverGoneLive</c> (verrou permanent) au lieu de <c>WentLiveAt IS NOT
    /// NULL</c> (affichage courant, remis à null par RevertToTestCommand), pour que les DEUX correctifs
    /// tiennent ensemble. AGENTS.md règle « ne jamais modifier une migration déjà appliquée » : on ne
    /// touche pas AddHasEverGoneLiveLock, on la corrige par une nouvelle migration qui s'applique après.
    ///
    /// <c>Down()</c> restaure l'état (erroné) d'AddHasEverGoneLiveLock, pour une réversibilité symétrique
    /// — jamais celui d'avant ExtendResetSchoolDataToConfiguration, qui n'a plus existé sur main.
    /// </summary>
    public partial class FixResetSchoolDataRegressionFromLiveModeGuard : Migration
    {
        /// <summary>Périmètre complet (données + configuration), identique à ExtendResetSchoolDataToConfiguration.</summary>
        ...
        {
            // Restaure l'état (erroné) laissé par AddHasEverGoneLiveLock — jamais celui d'avant
            // ExtendResetSchoolDataToConfiguration, qui n'a plus existé sur main une fois celle-ci fusionnée.
            migrationBuilder.Sql($$"""
                CREATE OR REPLACE FUNCTION reset_school_data(p_school_id uuid)
                RETURNS TABLE (label text, rows_deleted integer)
                LANGUAGE plpgsql
