...
{
    /// <summary>
    /// Refonte du garde-fou de la « Zone de danger », sur décision explicite du Directeur (produit) :
    /// le passage test ↔ réel (<c>WentLiveAt</c>) redevient une bascule PLEINEMENT réversible qui ne
    /// verrouille plus jamais la purge par effet de bord. <c>schools."HasEverGoneLive"</c> est renommée
    /// en <c>IsProductionLocked</c> — même colonne, même valeurs conservées — et ne sera plus posée que
    /// par une action manuelle et distincte : <c>LockProductionCommand</c> (« Verrouiller
    /// définitivement l'établissement »), qui horodate aussi <c>ProductionLockedAt</c>.
    ///
    /// Backfill DÉLIBÉRÉ (arbitrage produit du 19/09/2026, PAS une simple reprise de valeurs) :
    /// <c>IsProductionLocked</c> est forcé à <c>false</c> pour TOUTES les écoles existantes, y compris
    /// celles déjà passées en mode réel avant cette migration (donc déjà verrouillées sous l'ancien
    /// régime). Elles retrouvent la « Zone de danger » tant qu'un Directeur ne clique pas
    /// explicitement sur le nouveau bouton de verrouillage définitif — accepté en connaissance de
    /// cause pour rendre la main aux écoles déjà en exploitation, plutôt que de reproduire un verrou
    /// qu'elles n'ont jamais posé elles-mêmes.
    ///
    /// <c>reset_school_data</c> est réécrite en conséquence : verrou permanent sur
    /// <c>IsProductionLocked</c> (inchangé, sauf verrouillage explicite), ET nouvelle garde REVERSIBLE
    /// sur <c>WentLiveAt</c> (la purge n'a pas sa place en exploitation réelle, mais un retour en mode
    /// test la rouvre aussitôt) — ce second contrôle n'existait pas explicitement côté fonction avant
    /// cette migration, il ne reposait que sur la coïncidence de l'ancien verrou automatique.
    ///
    /// <c>Down()</c> restaure l'état FixResetSchoolDataRegressionFromLiveModeGuard /
    /// AddClassJournalToResetSchoolData (garde unique sur HasEverGoneLive).
    /// </summary>
    public partial class DecoupleProductionLockFromLiveMode : Migration
    {
        /// <summary>Périmètre de purge, identique à AddClassJournalToResetSchoolData (inchangé par cette migration).</summary>
        ...
        /// <summary>Nettoyage des comptes du personnel, identique à AddClassJournalToResetSchoolData.</summary>
        private const string StaffCleanup = """
        ...
        {
            // Même colonne, même valeurs : un Directeur qui avait déjà verrouillé (ancien régime
            // automatique) le reste tant que le backfill ci-dessous n'a pas tourné.
            migrationBuilder.RenameColumn(
                name: "HasEverGoneLive",
                table: "schools",
                newName: "IsProductionLocked");

            migrationBuilder.AddColumn<DateTimeOffset>(
                name: "ProductionLockedAt",
                table: "schools",
                type: "timestamp with time zone",
                nullable: true);

            // Backfill DÉLIBÉRÉ : déverrouille TOUTES les écoles existantes, y compris celles déjà
            // verrouillées sous l'ancien régime automatique (arbitrage produit du 19/09/2026 — voir le
            // commentaire de classe). Elles ne seront reverrouillées que par une action manuelle et
            // explicite du Directeur (LockProductionCommand).
            migrationBuilder.Sql("""UPDATE schools SET "IsProductionLocked" = false;""");
            ...
            migrationBuilder.Sql($$"""
                ...
                {{Targets}}
                    ...
                    v_deleted integer;
                    v_index integer;
                    v_is_production_locked boolean;
                ...
                BEGIN
                    ...
                    -- Verrou PERMANENT (DecoupleProductionLockFromLiveMode) : posé UNIQUEMENT par une
                    -- action manuelle et explicite du Directeur (LockProductionCommand), plus jamais par
                    -- effet de bord du passage en mode réel. Placé APRÈS les gardes de tenant.
                    SELECT s."IsProductionLocked" INTO v_is_production_locked FROM schools s WHERE s."Id" = p_school_id;

                    IF v_is_production_locked THEN
                        RAISE EXCEPTION
                            'RESET_UNAVAILABLE_PRODUCTION_LOCKED : l''établissement % a été verrouillé définitivement par un Directeur ; ses données ne peuvent plus être effacées.',
                            p_school_id
                            USING ERRCODE = 'raise_exception';
                    END IF;

                    -- Garde du mode COURANT, réversible : la purge n'a pas sa place en exploitation
                    -- réelle, mais un retour en mode test (RevertToTestCommand, disponible à tout
                    -- moment) la rouvre aussitôt — sauf le verrou ci-dessus.
                    SELECT s."WentLiveAt" INTO v_went_live FROM schools s WHERE s."Id" = p_school_id;

                    IF v_went_live IS NOT NULL THEN
                        RAISE EXCEPTION
                            'RESET_UNAVAILABLE_LIVE_MODE : l''établissement % est en mode réel depuis le % ; repassez en mode test pour réinitialiser.',
        ...
        {
            // Ordre inverse de Up() : la colonne doit reprendre son ancien nom AVANT que la fonction ne
            // la référence de nouveau sous ce nom.
            migrationBuilder.RenameColumn(
                name: "IsProductionLocked",
                table: "schools",
                newName: "HasEverGoneLive");

            migrationBuilder.DropColumn(
                name: "ProductionLockedAt",
                table: "schools");

            // État FixResetSchoolDataRegressionFromLiveModeGuard / AddClassJournalToResetSchoolData :
            // garde unique sur HasEverGoneLive, posée automatiquement par GoLiveCommandHandler.
            migrationBuilder.Sql($$"""
                CREATE OR REPLACE FUNCTION reset_school_data(p_school_id uuid)
                RETURNS TABLE (label text, rows_deleted integer)
                LANGUAGE plpgsql
