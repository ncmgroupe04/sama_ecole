...
/// (docs/Volume_7_Security.md §12bis, AGENTS.md règle #11). Étendu (ticket JGK-B03) au passage
/// automatique en LECTURE SEULE à expiration (<c>Status = ReadOnly</c>, voir
/// SubscriptionLifecycleHostedService) : contrairement à AwaitingPayment, ReadOnly ne bloque QUE les
/// requêtes mutantes (POST/PUT/PATCH/DELETE) — l'établissement garde un accès en lecture à ses
/// données pendant qu'il régularise son abonnement.
///
...
/// assurée côté client par wwwroot/js/api.js, qui intercepte les codes SUBSCRIPTION_AWAITING_PAYMENT/
/// SUBSCRIPTION_READ_ONLY ci-dessous exactement comme il intercepte déjà un 401.
/// </summary>
public class SubscriptionAwaitingPaymentMiddleware(RequestDelegate next)
{
    /// <summary>Verbes qui ne modifient rien : jamais bloqués en mode ReadOnly (ils le restent en AwaitingPayment).</summary>
    ...
    public async Task InvokeAsync(HttpContext context, IApplicationDbContext dbContext)
    {
        ...
        // (JGK-B01, ou données de démonstration/tests) — rien à restreindre. Un abonnement Active ne
        // restreint évidemment rien non plus.
        if (status is not (SubscriptionStatus.AwaitingPayment or SubscriptionStatus.ReadOnly))
        {
            await next(context);
            return;
        }

        // ReadOnly (ticket JGK-B03, expiration) ne bloque QUE les requêtes mutantes — l'établissement
        // garde un accès en lecture pendant qu'il régularise. AwaitingPayment, lui, bloque tout,
        // lecture comprise (§12bis) : un premier paiement jamais confirmé n'a jamais ouvert l'accès.
        if (status is SubscriptionStatus.ReadOnly && SafeMethods.Contains(context.Request.Method))
        {
            await next(context);
            return;
        }
        ...
        var payload = status is SubscriptionStatus.ReadOnly
            ? new
            {
                code = "SUBSCRIPTION_READ_ONLY",
                message = "Votre abonnement a expiré. L'accès est désormais en lecture seule tant que le renouvellement n'est pas confirmé.",
                details = (object?)null,
