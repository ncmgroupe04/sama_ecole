...
/// TeacherImportFileRow). Aucune validation MÉTIER ici (format de date, existence des matières...) —
/// seulement la structure du fichier : c'est ImportTeachersCommandHandler qui résout chaque ligne.
///
/// Même convention que StudentImportFileParser : la première ligne est TOUJOURS un en-tête.
/// </summary>
public class TeacherImportFileParser : ITeacherImportFileParser
{
    /// <summary>Nombre de colonnes attendu (voir TeacherImportFileRow) — les colonnes manquantes en fin
    /// de ligne (lieu de naissance non renseigné) sont complétées par des chaînes vides, jamais une erreur.</summary>
    private const int ColumnCount = 6;

    public IReadOnlyList<TeacherImportFileRow> Parse(byte[] fileContent, string fileName)
    {
        ...
        if (rows.Count == 0)
        {
            throw new ValidationException([
                new ValidationFailure("File", "Le fichier ne contient aucune ligne d'enseignant (au-delà de l'en-tête).")
            ]);
        }
    ...
    private static List<TeacherImportFileRow> ParseCsv(byte[] fileContent)
    {
        ...
        var rows = new List<TeacherImportFileRow>();
        ...
        for (var i = 0; i < lines.Length; i++)
        {
            ...
            rows.Add(new TeacherImportFileRow(
                i + 1, fields[0], fields[1], fields[2], fields[3], fields[4], fields[5]));
        }
    ...
    private static List<TeacherImportFileRow> ParseExcel(byte[] fileContent)
    {
        try
        {
            ...
            foreach (var xlRow in worksheet.RowsUsed())
            {
                ...
                rows.Add(new TeacherImportFileRow(
                    xlRow.RowNumber(), fields[0], fields[1], fields[2], fields[3], fields[4], fields[5]));
            }
    ...
    /// <summary>
    /// Une cellule de date de naissance peut être un VRAI type date Excel ou du texte simple : dans les
    /// deux cas, on ramène à la même représentation texte « jj/mm/aaaa » — même contrat que
    /// StudentImportFileParser.CellText.
    /// </summary>
